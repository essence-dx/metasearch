...
use crate::result_policy::ResultPolicy;
...
impl SearchOrchestrator {
    pub fn new(
        ...
        max_engines: usize,
        blocked_result_domains: Vec<String>,
    ) -> Self {
        Self {
            registry,
            cache,
            health,
            max_engines,
            result_policy: ResultPolicy::new(blocked_result_domains),
        }
    }
    ...
    async fn execute_search(&self, query: &SearchQuery) -> SearchResponse {
        ...
        let categories = if query.categories.is_empty() {
            vec![SearchCategory::General]
        } else {
            query.categories.clone()
        };

        // Honor explicit engine selection when provided; otherwise use the
        // category registry and skip circuit-broken engines.
        let requested_engines: Vec<Arc<dyn SearchEngine>> = if query.engines.is_empty() {
            self.registry.engines_for_categories(&categories)
        } else {
            query
                .engines
                .iter()
                .filter_map(|engine_name| self.registry.get(engine_name))
                .collect()
        };

        let engines: Vec<Arc<dyn SearchEngine>> = requested_engines
            .into_iter()
            .filter(|engine| {
            ...
            match outcome {
                Some((weight, results)) => {
                    ...
                    for mut r in results {
                        if !self.result_policy.allows(&r) {
                            continue;
                        }
