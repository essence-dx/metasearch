...
use metasearch_server::{
    app, cache::SearchCache, health::EngineHealthTracker, input, orchestrator::SearchOrchestrator,
...
enum Commands {
    ...
    /// Execute a live search and print the JSON API payload
    Search {
        /// Search query text
        #[arg(short, long)]
        query: String,
        ...
        /// Language code such as en, fr, or bn
        #[arg(long)]
        language: Option<String>,
        /// Result page number
        #[arg(long)]
        page: Option<u32>,
        /// Safe search level: 0 off, 1 safe, 2 strict
        #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let blocked_result_domains = settings.search.blocked_result_domains.clone();
    ...
    match cli.command.unwrap_or(Commands::Serve) {
        ...
        Commands::Search {
            ...
            if !allow_network {
                anyhow::bail!("Search makes live network calls. Re-run with --allow-network.");
            }
            run_search_json(
                &state,
                SearchCommandParams {
...
async fn run_search_json(state: &Arc<AppState>, params: SearchCommandParams) -> anyhow::Result<()> {
    let Some(query_text) = input::normalize_query(&params.query, input::MAX_QUERY_CHARS) else {
        anyhow::bail!("The --query value is required.");
    };

    if let Some(format) = params.format.as_deref() {
        if !format.eq_ignore_ascii_case("json") {
            anyhow::bail!("Only --format json is supported.");
        }
    }
