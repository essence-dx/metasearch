...
impl RawDxConfig {
    ...
    fn absolutize(&mut self, base: &Path) {
        ...
        self.paths.cache = abs_path(&root, &self.paths.cache);
    }
}
...
impl Default for PathsConfig {
    fn default() -> Self {
        Self {
            cache: PathBuf::from(".dx/cache"),
        }
    }
}

pub struct MetasearchDxConfig {
    pub workspace_root: PathBuf,
    pub sr_dir: PathBuf,
    pub receipts_dir: PathBuf,
}

impl MetasearchDxConfig {
    pub fn load() -> Self {
        let raw = RawDxConfig::load(&std::env::current_dir().unwrap_or_default());
        let ws = raw.workspace.root;
        let sr = ws.join(".dx").join("serializer");
        let receipts = ws.join(".dx").join("receipts").join("metasearch");
        Self {
            workspace_root: ws,
            sr_dir: sr,
