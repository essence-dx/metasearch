...
#[test]
fn sensitive_social_adapters_are_not_registered_by_default() {
    let registry = EngineRegistry::with_defaults(Client::new());

    assert!(
        registry.get("9gag").is_none(),
        "9GAG should not be available in the default registry"
    );
    assert!(
        registry.engine_names().iter().all(|name| name != "9gag"),
        "9GAG should not appear in the default engine list"
    );
}
